...
from fairseq.tasks.utterance_mixing_pretraining_aug import (
    UtteranceMixingPretrainingConfig,
    UtteranceMixingPretrainingTask_AUG,
)
...
@register_model("hubert_aug", dataclass=HubertConfig)
class HubertModel(BaseFairseqModel):
    def __init__(
        ...
        super().__init__()
        logger.info(f"HubertModel_AUG Config: {cfg}")
    ...
    def forward(
        ...
        if features_only:
            return {"x": x, "padding_mask": padding_mask, "features": features, "layerwise": _}
    ...
    def extract_features(
        ...
        layerwise_feature = res['layerwise']
        return feature, res["padding_mask"]

    def extract_features_layer(
        self,
        source: torch.Tensor,
        padding_mask: Optional[torch.Tensor] = None,
        mask: bool = False,
        ...
        feature = res["features"] if ret_conv else res["x"]
        layerwise_feature = res['layerwise']
        return layerwise_feature, res["padding_mask"]
    ...
    # def extract_layerwise_features(self, source):
