...
from fairseq.modules import MultiheadAttention_wavlm as MultiheadAttention
...
class WavLMModel(BaseFairseqModel):
    def __init__(
        self,
        cfg: WavLMConfig,
        ...
        super().__init__()
        logger.info(f"WavLMModel Config: {cfg}")
        self.cfg = cfg
    ...
    @classmethod
    def build_model(cls, cfg: WavLMConfig, task: UtteranceMixingPretrainingTask):
        """Build a new model instance."""

        model = WavLMModel(cfg, task.cfg, task.dictionaries)
        return model
    ...
    def extract_features_layer(
        ...
        # feature = res["features"] if ret_conv else res["x"]
        layerwise_feature = res['layer_results']
