...
class AugmentWAV(object):
    ...
    def additive_noise(self, noisecat, audio_torch):

        self.max_audio = audio_torch.shape[1]
        ...
        for noise in noiselist:
            ...
            noiseaudio  = loadWAV(noise, self.max_audio, evalmode=False)
            noiseaudio = noiseaudio.astype(np.float32) / np.iinfo(np.int16).max
            noise_snr   = random.uniform(self.noisesnr[noisecat][0],self.noisesnr[noisecat][1])
            noise_db = 10 * np.log10(np.mean(noiseaudio[0] ** 2)+1e-4) 
            noises.append(np.sqrt(10 ** ((clean_db - noise_db - noise_snr) / 10)) * noiseaudio)

        return torch.from_numpy(np.sum(np.concatenate(noises,axis=0),axis=0,keepdims=True) + audio).type_as(audio_torch)

    def reverberate(self, audio_torch):
        ...
        rir = rir.astype(np.float32) / np.iinfo(np.int16).max
        ...
        return torch.from_numpy(signal.convolve(audio, rir, mode='full')[:,:self.max_audio]).type_as(audio_torch)
...
class UtteranceMixingDataset_AUG(FairseqDataset):
    def __init__(
        self,
        manifest_path: str,
        sample_rate: float,
    ...
    def batch_by_size(self, indices, max_tokens=None, max_sentences=None, required_batch_size_multiple=1):
        ...
        if isinstance(indices[0], list):
            ...
            for indice in indices:
                batch = super(UtteranceMixingDataset_AUG, self).batch_by_size(indice, max_tokens, max_sentences, required_batch_size_multiple)
                batch_list.append(batch)
            return batch_list
        else:
            return super(UtteranceMixingDataset_AUG, self).batch_by_size(indices, max_tokens, max_sentences, required_batch_size_multiple)
    ...
    def get_audio(self, index):
        ...
        # wav, cur_sample_rate = sf.read(wav_path,samplerate=16000)
        wav = get_features_or_waveform(wav_path, need_waveform=True, use_sample_rate=self.sample_rate)
        get_features_or_waveform
        wav = torch.from_numpy(wav).float()
        # TODO we enforce the sample rate as 16k
        wav = self.postprocess(wav, 16000)
        return wav
